...
export default function OrderBarChart({ activeCard, products = [], onClose }) {
  ...
  const activeKey = useMemo(() => {
    return Object.keys(CARD_THEMES).find(k => CARD_THEMES[k].title === activeCard) || "total";
  }, [activeCard]);
  ...
  const chartData = useMemo(() => {
    ...
    const filtered = products.filter(p => (p[activeKey] || 0) > 0);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: "20px" }}>
        <div>
          <h3 style={{ fontSize: "16px", fontWeight: "600", color: "#111827", margin: 0 }}>
            {theme.title} Breakdown by Product (Item Quantities)
          </h3>
          <p style={{ margin: "4px 0 0 0", fontSize: "12px", color: "#6b7280" }}>
            Showing product quantity contributions to the total {theme.title.toLowerCase()} ({chartData.totalValue.toLocaleString()} items)
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {visibleProducts.map((p, idx) => {
          ...
          const successPct = p.total > 0 ? ((val / p.total) * 100).toFixed(1) : "0.0";
            ...
            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: '1 1 200px', minWidth: '200px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                <span style={{ fontSize: '13px', fontWeight: '700', color: '#111827' }}>
                  {val.toLocaleString()} {val === 1 ? 'item' : 'items'}
                </span>
